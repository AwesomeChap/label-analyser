...
const CROP_TRANSCRIBE_PROMPT = `Transcribe all visible printed text on this image (labels, numbers, line breaks as needed). Ignore QR codes, barcodes, and Data Matrix symbols — do not decode them, only human-readable text lines.
Output plain text only. No JSON, no markdown, no quotes around the answer. If there is no readable text, output a single hyphen -.`;

/**
 * Single-region OCR for a cropped label image (used with YOLO OBB pipeline).
 */
export async function transcribeImageCrop(buffer, mimeType, apiKey) {
  const ai = new GoogleGenAI({ apiKey });
  const model = process.env.GEMINI_MODEL || 'gemini-2.5-flash';
  const base64 = buffer.toString('base64');
  const contents = [
    { inlineData: { mimeType: mimeType || 'image/jpeg', data: base64 } },
    { text: CROP_TRANSCRIBE_PROMPT },
  ];
  const response = await ai.models.generateContent({
    model,
    contents,
